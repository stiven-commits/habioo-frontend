...
export default function EstadoCuentasBancarias() {
    ...
    try {
      ...
      if (data.status === 'success') {
        ...
        if (data.bancos.length > 0) {
          const predeterminada = data.bancos.find(c => c.es_predeterminada) || data.bancos[0];
          setSelectedCuenta(predeterminada.id.toString());
        }
      }
  ...
  const fondosCuenta = fondos.filter((f) => f.cuenta_bancaria_id?.toString() === selectedCuenta);
  const saldoCuentaUsdActual = fondosCuenta.reduce((acc, f) => {
    const saldo = parseFloat(f.saldo_actual || 0);
    if (f.moneda === 'USD') return acc + saldo;
    if (f.moneda === 'BS' && tasaBcvNum > 0) return acc + (saldo / tasaBcvNum);
    return acc;
  }, 0);
  const saldoCuentaBsActual = fondosCuenta.reduce((acc, f) => {
    const saldo = parseFloat(f.saldo_actual || 0);
  ...
  const tablaConSaldos = movimientosOrdenados.map(mov => {
    const montoBs = parseFloat(mov.monto_bs || 0);
    const tasa = parseFloat(mov.tasa_cambio || 0);
    let montoUsd = parseFloat(mov.monto_usd || 0);
    ...
    if (activeFondoId && isInterna) {
      const esOrigen = parseInt(mov.fondo_origen_id || 0, 10) === activeFondoId;
      const esDestino = parseInt(mov.fondo_destino_id || 0, 10) === activeFondoId;
      isEntrada = esDestino;
      isSalida = esOrigen;
    }
  ...
  // Invertir el array para mostrar los más recientes arriba
  const tablaInvertida = [...tablaConSaldos].reverse();
  const cuentaActual = cuentas.find(c => c.id.toString() === selectedCuenta);
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      ...
      {/* 💡 PANEL SUPERIOR: SELECTOR Y BOTONES GLOBALES */}
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            ...
            <div className="flex items-end gap-2">
              <div>
                ...
                <input type="date" value={fechaDesde} onChange={(e) => setFechaDesde(e.target.value)} className="p-2 border border-gray-200 dark:border-gray-700 rounded-lg text-xs bg-gray-50 dark:bg-gray-800 outline-none dark:text-white" />
              </div>
              <div>
                <label className="block text-[10px] uppercase font-bold text-gray-400 mb-1">Hasta</label>
                <input type="date" value={fechaHasta} onChange={(e) => setFechaHasta(e.target.value)} className="p-2 border border-gray-200 dark:border-gray-700 rounded-lg text-xs bg-gray-50 dark:bg-gray-800 outline-none dark:text-white" />
              </div>
