  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="bg-white dark:bg-donezo-card-dark rounded-3xl p-6 w-full max-w-3xl shadow-2xl border border-gray-100 dark:border-gray-800 relative my-8">
        <button onClick={() => setIsModalOpen(false)} className="absolute top-4 right-4 text-gray-400 hover:text-red-500 font-bold text-xl">âœ•</button>
        <h3 className="text-2xl font-bold text-gray-800 dark:text-white mb-6">{editingId ? 'âœï¸ Editar Inmueble' : 'ðŸ  Nuevo Inmueble'}</h3>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-fadeIn">
      <div className="bg-white dark:bg-donezo-card-dark rounded-3xl p-6 w-full max-w-md shadow-2xl relative">
        <button onClick={() => setAjusteModalOpen(false)} className="absolute top-4 right-4 text-gray-400 hover:text-red-500 font-bold text-xl">âœ•</button>
        <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-2 flex items-center gap-2">âš–ï¸ Ajustar Saldo</h3>
        ...
        <form onSubmit={handleSubmitAjuste} className="space-y-4">
          <div>
            <label className="block text-xs font-bold text-gray-500 uppercase mb-1">AcciÃ³n a realizar</label>
            <select value={formAjuste.tipo_ajuste} onChange={e => setFormAjuste({ ...formAjuste, tipo_ajuste: e.target.value })} className="w-full p-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-sm font-bold outline-none focus:ring-2 focus:ring-donezo-primary dark:text-white">
              <option value="CARGAR_DEUDA">ðŸ”´ Cargar Deuda (+)</option>
              <option value="AGREGAR_FAVOR">ðŸŸ¢ Agregar a Favor (-)</option>
            </select>
          </div>
          ...
          <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">Nota (AuditorÃ­a) *</label><textarea value={formAjuste.nota} onChange={e => setFormAjuste({ ...formAjuste, nota: e.target.value })} placeholder="Ej: Cobro de multa" className="w-full p-3 rounded-xl border dark:bg-gray-900 dark:border-gray-700 outline-none dark:text-white text-sm min-h-[80px]" required /></div>
...
// ðŸ’¡ MODAL PARA CARGA MASIVA DE EXCEL CON PASO A PASO
// ðŸ’¡ MODAL PARA CARGA MASIVA DE EXCEL
export function ModalCargaMasiva({
  ...
  loteErrors,
  isUploadingLote,
  uploadProgress, // ðŸ’¡ Recibe el progreso
        ...
        <div className="p-6 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center bg-gray-50 dark:bg-gray-900/50">
           <div>
              <h3 className="text-xl font-black text-gray-800 dark:text-white flex items-center gap-2">
                ðŸ“Š Carga Masiva de Inmuebles
              </h3>
              {loteData.length > 0 && (
           ...
           {/* ðŸ’¡ BotÃ³n X desactivado durante la carga */}
           <button 
             onClick={handleClose} 
        ...
        {loteData.length === 0 ? (
          <div className="p-10 flex flex-col items-center justify-center bg-white dark:bg-donezo-card-dark min-h-[300px]">
            ...
            <p className="text-gray-500 dark:text-gray-400 text-center max-w-md mb-8">
              Para cargar mÃºltiples propiedades de golpe, descarga nuestra plantilla de Excel, llÃ©nala con los datos y sÃºbela al sistema. Las cÃ©dulas se usarÃ¡n como claves temporales.
            </p>
          ...
          <>
            <div className="flex-1 overflow-y-auto p-0 bg-white dark:bg-donezo-card-dark custom-scrollbar relative">
              {/* ðŸ’¡ OVERLAY DE BLOQUEO PARA LA TABLA MIENTRAS CARGA */}
              {isUploadingLote && (
              ...
              <table className="w-full text-left border-collapse text-sm">
                <thead className="sticky top-0 bg-gray-100 dark:bg-gray-800 shadow-sm z-10">
                  <tr className="text-gray-600 dark:text-gray-300">
                    ...
                    <th className="p-3 font-bold">CÃ©dula</th>
                    <th className="p-3 font-bold">Correo</th>
                    <th className="p-3 font-bold">TelÃ©fono</th>
                    <th className="p-3 font-bold text-right">AlÃ­cuota</th>
                ...
                <tbody>
                  {loteData.map((row, i) => (
                    <tr key={i} className={`border-b ${row.isValid ? 'border-gray-50 dark:border-gray-800/50 hover:bg-gray-50 dark:hover:bg-gray-800' : 'bg-red-50 dark:bg-red-900/10 border-red-100 dark:border-red-900/30'}`}>
                      <td className="p-3 text-center">{row.isValid ? <span className="text-green-500 text-lg" title="Correcto">âœ…</span> : <span className="text-red-500 text-lg cursor-help" title={row.errors}>âŒ</span>}</td>
                      ...
                      <td className="p-3 font-mono text-gray-600 dark:text-gray-400">{row.cedula}{!row.isValid && row.errors.includes('CÃ©dula') && <div className="text-[10px] text-red-500 font-bold">InvÃ¡lida</div>}</td>
                      ...
                      <td className="p-3 text-right font-mono font-bold text-blue-600 dark:text-blue-400">{String(row.alicuota).replace('.', ',')}% {!row.isValid && row.errors.includes('AlÃ­cuota') && <div className="text-[10px] text-red-500 font-bold">Debe ser {'>'} 0</div>}</td>
